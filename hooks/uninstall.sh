#!/bin/bash
# pechernyi — uninstaller for the SessionStart + UserPromptSubmit hooks
# Removes: hook files in ~/.claude/hooks, settings.json entries, and the flag file
# Usage: bash hooks/uninstall.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/Kerber0ss/pechernyi/main/hooks/uninstall.sh)
set -e

CLAUDE_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
HOOKS_DIR="$CLAUDE_DIR/hooks"
SETTINGS="$CLAUDE_DIR/settings.json"
FLAG_FILE="$CLAUDE_DIR/.pechernyi-active"

HOOK_FILES=("package.json" "pechernyi-config.js" "pechernyi-activate.js" "pechernyi-mode-tracker.js" "pechernyi-statusline.sh")

# Detect if pechernyi is installed as a plugin (check plugin cache)
PLUGIN_INSTALLED=0
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/pechernyi*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi
...
echo "Uninstalling pechernyi hooks..."
...
# 2. Remove pechernyi entries from settings.json (idempotent)
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the pechernyi SessionStart and UserPromptSubmit"
    ...
    PECHERNYI_SETTINGS="$SETTINGS" PECHERNYI_HOOKS_DIR="$HOOKS_DIR" node -e "
      ...
      // Remove statusLine if it references pechernyi
      if (settings.statusLine) {
        ...
        if (cmd.includes(managedStatusLinePath)) {
          ...
          console.log('  Removed pechernyi statusLine from settings.json');
        }
      }

      fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2) + '\n');
      console.log('  Removed ' + removed + ' pechernyi hook entries from settings.json');
    "
  fi
fi
...
echo ""
echo "Other agents:"
echo "  npx skills remove pechernyi    # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable pechernyi  # Claude Code plugin"
echo "  gemini extensions uninstall pechernyi  # Gemini CLI"
