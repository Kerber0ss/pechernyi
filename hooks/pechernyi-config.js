#!/usr/bin/env node
// pechernyi — shared configuration resolver
//
// Resolution order for default mode:
//   1. PECHERNYI_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/pechernyi/config.json (any platform, if set)
//      - ~/.config/pechernyi/config.json (macOS / Linux fallback)
//      - %APPDATA%\pechernyi\config.json (Windows fallback)
//   3. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'pechernyi');
  }
  ...
  return path.join(os.homedir(), '.config', 'pechernyi');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.PECHERNYI_DEFAULT_MODE;
...
// flag path (~/.claude/.pechernyi-active) with a symlink to clobber other files.
//
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(flagDir, `.pechernyi-active.${process.pid}.${Date.now()}`);
