...
const slashRegex = /^\/pechernyi(?:\s+(lite|full|ultra|off|status))?$/i;
...
process.stdin.on('end', () => {
  try {
    ...
    if (slashMatch) {
      ...
      if (arg === 'status') {
        const current = readFlag() || 'off';
        const statusText = current === 'off'
          ? 'Печерний вимкнений. Увімкнути: /pechernyi або /pechernyi lite|full|ultra'
          : 'Печерний активний. Режим: ' + current + '. Вимкнути: /pechernyi off';
        process.stdout.write(JSON.stringify({
          hookSpecificOutput: {
            hookEventName: "UserPromptSubmit",
            additionalContext: "PECHERNYI STATUS: " + current + ". " + statusText
          }
        }));
        process.exit(0);
      }
